            ...
            <div className="relative z-10 container mx-auto px-4 text-center text-white flex flex-col items-center">
                ...
                >
                    {hero.title.split(' with ').map((part: string, i: number) => (
                        <span key={i}>
                            {i === 1 ? <><br /> with <span className="italic text-gradient-gold">{part}</span></> : part}
                        </span>
                    ))}
                ...
                >
                    <Link
                        href="/services"
                        className="group relative px-8 py-4 bg-gradient-gold text-white text-lg font-bold rounded-full overflow-hidden shadow-[0_0_20px_rgba(212,175,55,0.3)] hover:shadow-[0_0_30px_rgba(212,175,55,0.5)] transition-all transform hover:-translate-y-1"
                    >
