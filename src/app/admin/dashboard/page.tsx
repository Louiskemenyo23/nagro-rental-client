...
const StatCard = ({ title, value, icon: Icon, isPrimary = false, subtext }: any) => (
    <div className={`p-6 rounded-3xl shadow-md border transition-all duration-300 hover:-translate-y-1 ${isPrimary
        ? "bg-[#101F3C] border-[#101F3C] text-white"
        : "bg-white border-gray-100 text-gray-800"
        }`}>
        <div className="flex justify-between items-start mb-4">
            <div>
                <p className={`text-sm font-medium ${isPrimary ? "text-blue-200" : "text-gray-400"}`}>{title}</p>
                <h3 className="text-3xl font-bold mt-2 tracking-tight">{value}</h3>
            </div>
            <div className={`w-10 h-10 rounded-full flex items-center justify-center ${isPrimary ? "bg-white/10 text-white" : "bg-orange-50 text-orange-500"
                }`}>
                <Icon size={20} />
            </div>
        </div>
        {subtext && <p className={`text-xs ${isPrimary ? "text-blue-300" : "text-gray-400"}`}>{subtext}</p>}
    </div>
);
    ...
    return (
        <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 h-full flex flex-col">
            <div className="flex justify-between items-center mb-8">
                ...
                <button
                    onClick={onCheck}
                    className="bg-orange-50 text-orange-500 px-4 py-1.5 rounded-lg text-xs font-bold hover:bg-orange-100 transition-colors"
                >
                    View Details
                </button>
            </div>
                ...
                {monthlyData.map((val, i) => {
                    ...
                    return (
                        <div key={i} className="flex flex-col items-center gap-2 group flex-1 h-full justify-end">
                            {/* Tooltip */}
                            <div className="opacity-0 group-hover:opacity-100 mb-2 bg-[#101F3C] text-white text-[10px] px-2 py-1 rounded absolute -mt-8 transition-opacity z-10 whitespace-nowrap">
                                GH₵{val.toLocaleString()}
                            </div>
                            <div
                                className={`w-full rounded-t-sm transition-all duration-500 ${val > 0 ? "bg-[#101F3C] group-hover:bg-orange-500" : "bg-gray-100"}`}
    ...
    <div className="bg-white p-8 rounded-3xl shadow-sm border border-gray-100 flex flex-col items-center justify-center relative h-full">
        <div className="relative w-40 h-40">
            <svg className="w-full h-full transform -rotate-90">
                ...
                <circle
                    cx="80" cy="80" r="70" stroke="#101F3C" strokeWidth="15" fill="none"
                    strokeDasharray="440"
            ...
            <div className="absolute inset-0 flex items-center justify-center flex-col">
                <span className="text-3xl font-bold text-[#101F3C]">{Math.round(pendingPercentage)}%</span>
        ...
        <div className="mt-8 text-center space-y-2 w-full">
            ...
            <button
                onClick={onCheck}
                className="bg-orange-500 text-white px-6 py-2.5 rounded-xl text-sm font-bold shadow-lg shadow-orange-500/20 hover:bg-orange-600 transition-all w-full"
            >
                Review Bookings
            </button>
        </div>
    ...
    <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 h-80 flex flex-col overflow-hidden relative">
        <div className="flex justify-between items-center mb-4 z-10">
            <div>
                ...
                <div className="flex items-center gap-2 mt-1">
                    <span className="w-2 h-2 rounded-full bg-[#101F3C]"></span>
        ...
        <div className="absolute bottom-0 left-0 right-0 h-40">
            <svg viewBox="0 0 1440 320" className="w-full h-full" preserveAspectRatio="none">
                <path fill="#F97316" fillOpacity="0.2" d="M0,192L48,197.3C96,203,192,213,288,229.3C384,245,480,267,576,250.7C672,235,768,181,864,181.3C960,181,1056,235,1152,234.7C1248,235,1344,181,1392,154.7L1440,128L1440,320L1392,320C1344,320,1248,320,1152,320C1056,320,960,320,864,320C768,320,672,320,576,320C480,320,384,320,288,320C192,320,96,320,48,320L0,320Z"></path>
                <path fill="#101F3C" fillOpacity="0.8" d="M0,160L48,170.7C96,181,192,203,288,202.7C384,203,480,181,576,165.3C672,149,768,139,864,154.7C960,171,1056,213,1152,218.7C1248,224,1344,192,1392,176L1440,160L1440,320L1392,320C1344,320,1248,320,1152,320C1056,320,960,320,864,320C768,320,672,320,576,320C480,320,384,320,288,320C192,320,96,320,48,320L0,320Z"></path>
            </svg>
        </div>
    </div>
);
        ...
        <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 h-80 flex flex-col">
                    ...
                    return (
                        <div key={i} className="relative flex flex-col items-center justify-center p-1">
                            <div className={`w-8 h-8 flex items-center justify-center rounded-full transition-all
                                ${isToday ? "bg-[#101F3C] text-white font-bold shadow-md" :
                                    bookingCount > 0 ? "bg-orange-50 text-orange-600 font-bold border border-orange-100" : "hover:bg-gray-50 text-gray-600"}
                            `}
            ...
            <div className="mt-2 flex items-center justify-end gap-3 text-[10px] text-gray-400">
                <div className="flex items-center gap-1">
                    <span className="w-1.5 h-1.5 rounded-full bg-[#101F3C]"></span> Today
                </div>
                <div className="flex items-center gap-1">
                    <span className="w-1.5 h-1.5 rounded-full bg-orange-500"></span> Event
                </div>
            </div>
        </div>
    );
